...
def main(config):
  ...
  xx,yy = np.meshgrid(list(range(64)), list(range(64)))
  i = 0
  mmin = 0
  mmax = 0
  for samp in data['X_train']:
    i+=1
    for dat in samp:
      dat = np.sqrt(dat[...,0]**2 + dat[...,0]**2)
      m = np.min(dat)
      if m < mmin:
        mmin = m 
      ...
      if M > mmax:
        mmax = M
      plt.contourf(xx, yy, dat, cmap = 'tab20c', levels = 50, vmin = 0, vmax = 9)
